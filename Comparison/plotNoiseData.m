...
function obj = plotNoiseData( obj, cam_sd_vals, scan_sd_vals, Nobs_vals, N_sim )           
    ...
    for idx_pattern = 1:size(patterns,1)
        ...
        for idx_method = 1:length(methods)
            ...
            for idx_sd = 1:size(cam_idxs,2)
                Rt(idx_sd,:) = [obj.(pat).(met).mem(idx_sd,idx_sd,Nobs_idxs,:)];         
            end

%             Rt = squeeze(obj.(pat).(met).mem(cam_idxs,cam_idxs,Nobs_idxs,:));
%             if size(Rt,2)==1
