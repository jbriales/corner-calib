...
% (2*) as an alternative classes can begin with C letter (uppercase) and be
% stored in parent folder
...
baseClasses = cellfun( @(x)x(1:end-2), baseClasses, 'UniformOutput', false );
...
%parent directory. 
        info = what(directory);
        for i=1:numel(info.m)
            if info.m{i}(1)=='C'
                info.classes{end+1} = info.m{i};
            end
        end
        flist = dir(directory);
