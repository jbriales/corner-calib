...
userOpts = readConfigFile( fullfile(pwd,'main.ini'), '[GT]' );
...
if strcmp(stereoLabel,'_L')
    R_ort = RotationY(deg2rad(-1))* [ -0.3434   -0.9381    0.0443
...
% Happy idea
R_ort = [  -0.3665   -0.9298    0.0349
   -0.0559   -0.0155   -0.9983
    0.9287   -0.3679   -0.0463 ];
t_3D = [ 0.4256 ; -0.0588 ; 0.0739 ];
...
%     reprojectScan2Img( im_gt, xy_gt, K, [R_m t_m] );
    hold on
%     reprojectScan2Img( Cam.img, LRF.xy, Cam.K, [R_ort_0 t_3D_0], false );
   ...
   if 0
       ...
       Npts = size(pts,2);
       delete(hPts);
       t_3D = t_3D_0 + [ +0.08 ; 0 ; 0.00 ];
       R_ort = RotationY(deg2rad(-4)) * RotationZ(deg2rad(-0)) * R_ort_0;
